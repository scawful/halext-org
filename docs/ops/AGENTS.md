...
Open **Admin → Server & Projects** inside the SPA to view:
- live uptime/load/memory/disk stats pulled from `/admin/server/status`,
- auto-refreshed service health (`halext-api`, nginx, PostgreSQL, OpenWebUI, Ollama),
- git metadata (branch + commit) for the running code,
- curated quick links to the docs/ and scripts/ indexes so you can hop between operational tasks without digging through the repo.

Use the **Refresh status** button before/after a deploy to confirm resource usage and service states without SSHing into the box.

## SSH Access

- `ssh halext-server` – alias defined in `~/.ssh/config` (`HostName org.halext.org`, `User halext`, `IdentityFile ~/.ssh/id_rsa`)
- `ssh halext-server 'uptime'` – run single commands without opening an interactive shell
- `ssh -t halext-server 'sudo systemctl restart halext-api'` – add `-t` whenever sudo needs to prompt for a password
- Raw form (`ssh halext@org.halext.org`) still works if you need to override the alias
- Keys live at `~/.ssh/id_rsa*`; update `/home/halext/.ssh/authorized_keys` if the key ever changes
- Use `./scripts/setup-ssh-key.sh` for copy/paste instructions

## Distributed AI Router

- `GET /ai/models` lists every available provider (OpenAI, Gemini, OpenWebUI, remote Ollama nodes); identifiers look like `openai:gpt-4o-mini` or `client:<node_id>:llama3.1`
- `AI_DEFAULT_MODEL=client:<node_id>:<model>` pins the default route to a remote box (avoid burning CPU on the Ubuntu VM)
- Admins can add/remove remote workers via `/admin/ai-clients`; nodes marked `is_public=true` are visible to all users and show up automatically in `/ai/models`
- Client devices must listen on Ollama/OpenWebUI and be reachable via Tailscale/port-forwarding (see `docs/CONNECTING_MAC_TO_UBUNTU.md`)
- API responses now echo the resolved model id; check `X-Halext-AI-Model` on `/ai/stream` when debugging
