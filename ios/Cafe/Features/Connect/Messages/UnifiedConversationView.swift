...
struct UnifiedConversationView: View {
    ...
    private var conversationMenu: some View {
        ...
        .accessibilityHint("Opens menu with options to change AI model, regenerate response, or clear messages")
    }
    ...
    private var inputArea: some View {
        HStack(spacing: 12) {
            TextField(conversation.isAIEnabled ? "Ask me anything..." : "Type a message...", text: $inputText, axis: .vertical)
                ...
                .accessibilityLabel(conversation.isAIEnabled ? "Message input for AI assistant" : "Message input")
                .accessibilityHint("Enter your message here. Press return or tap send to submit.")
                ...
                Image(systemName: isLoading || isStreaming ? "stop.circle.fill" : "arrow.up.circle.fill")
                    ...
                    .foregroundStyle(
                        inputText.isEmpty && !isLoading && !isStreaming
                            ? AnyShapeStyle(themeManager.secondaryTextColor)
                            : AnyShapeStyle(themeManager.accentColor)
                    )
            }
            .disabled(inputText.isEmpty && !isLoading && !isStreaming)
            .accessibilityLabel(isLoading || isStreaming ? "Stop generating" : "Send message")
            .accessibilityHint(isLoading || isStreaming ? "Stops the current AI response" : "Sends your message to the conversation")
...
struct MessageBubbleView: View {
    ...
    private var accessibilityMessageLabel: String {
        let sender = message.isFromAI ? "AI Assistant" : "You"
        let timeFormatter = DateFormatter()
        timeFormatter.timeStyle = .short
        let time = timeFormatter.string(from: message.createdAt)
        return "\(sender) said: \(message.content). Sent at \(time)"
    }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: message.isFromAI ? .leading : .trailing, spacing: 4) {
                ...
                Text(message.content)
                    .font(.body)
                    .foregroundColor(message.isFromAI ? themeManager.textColor : .white)
                    .padding(.horizontal, 16)
                    .padding(.vertical, 12)
                    .background(
                        RoundedRectangle(cornerRadius: 20)
                            .fill(message.isFromAI ? themeManager.secondaryBackgroundColor : themeManager.accentColor)
                    )
                    .contextMenu {
                        Button {
        ...
        .accessibilityHint("Double tap and hold for options to copy or share")
    }
}
...
struct StreamingMessageBubble: View {
    ...
    private var accessibilityLabel: String {
        if text.isEmpty {
            ...
            return "AI Assistant is responding: \(text)"
        }
    }
