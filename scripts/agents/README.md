...
| `ai-health-auth.sh` | Wrapper that pulls the access code from `/srv/halext.org/agent-notes/ai-health-token.txt` (or `HAL_AI_CODE`) and bearer token from `/tmp/test-token.txt` (or `HAL_AI_BEARER[_FILE]`), then runs `ai-health.sh` with both headers set. |
| `ios-api-smoke.sh` | iOS-centric smoke probe hitting `/api/health`, `/users/me/`, `/ai/provider-info`, `/ai/models`, `/conversations/`, and the first conversation’s messages (if any); supports `HAL_API_CODE`/`HAL_AI_CODE` and bearer token env/file. |
